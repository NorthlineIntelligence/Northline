...
export default async function AdminAssessmentDashboardPage(props: {
  ...
  // Exclude the admin “owner” participant row from reporting (prevents skew).
  ...
  // Started = either invite accepted OR at least 1 response
  ...
  // --- Results snapshot (uses your single source of truth engine) ---
  const results = await buildAssessmentResultsPayload({ assessmentId });
  const resultsOk = results?.ok === true;

  const overallScore = resultsOk ? results.body.aggregate.overall.weightedAverage : null;
  const overallBand = resultsOk ? results.body.bands.overall.band : null;
  const overallBandColor = resultsOk ? results.body.bands.overall.color : "#cdd8df";

  const pillarRows = resultsOk
    ? Object.entries(results.body.aggregate.pillars).map(([pillarKey, pillarObj]) => {
        const bandObj = results.body.bands.pillars[pillarKey];
        return {
          key: pillarKey,
          score: pillarObj.weightedAverage,
          band: bandObj?.band ?? null,
          color: bandObj?.color ?? "#cdd8df",
          coveragePct: pillarObj.datasetCoveragePct,
        };
      })
    : [];

  // --- Question heatmap (server-side aggregation) ---
  ...
  const pillarOrder = ["SYSTEM_INTEGRITY", "HUMAN_ALIGNMENT", "STRATEGIC_COHERENCE", "SUSTAINABILITY_PRACTICE"];
    ...
    <div className="min-h-screen bg-[#fcfcfe] text-[#173464]">
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mt-6 rounded-2xl border border-[#e6eaf2] bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-7 items-stretch">
            <div className="md:col-span-2 rounded-2xl border border-[#e6eaf2] bg-[#fcfcfe] p-5">
            ...
            <div className="hidden md:flex items-center justify-center text-2xl text-[#66819e]">→</div>
            ...
            <>
              <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-[#e6eaf2] bg-[#fcfcfe] p-5">
                  ...
                  <div className="mt-1 text-3xl font-semibold text-[#173464]">{fmt1(overallScore)}</div>
                  <div
                  ...
                  >
                    <span
                      style={{
                        ...
                        height: 10,
                        borderRadius: 999,
                        backgroundColor: overallBandColor,
                        display: "inline-block",
                      }}
                    />
                    <span className="text-[#173464]">{overallBand ?? "—"}</span>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-[#e6eaf2] bg-[#fcfcfe] p-5 md:col-span-2">
                  ...
                  <div className="mt-3 text-xs text-[#66819e]">
                    Coverage is based on available responses in the dataset.
                  </div>
                </div>
              </div>
            </>
        ...
        <div className="mt-8 rounded-2xl border border-[#e6eaf2] bg-white p-6 shadow-sm">
                  ...
                  {displayedHeatmapRows.map((r) => (
                    <tr key={r.question_id} className="border-t border-[#e6eaf2]">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-[#66819e] font-mono">{r.question_id}</div>
                      </td>
          ...
          <div className="mt-3 text-xs text-[#66819e]">
            Interpretation tip: low average + high coverage = real weakness. Low average + low coverage = needs more
            completions.
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-[#e6eaf2] bg-white p-4 text-xs text-[#66819e]">
          Tip: “Started” counts anyone who accepted the invite or answered at least one question. This is the best early
          signal for participation momentum.
        </div>
      </div>
    </div>
  );
